/*
   Function to display DOM content (A UTF-8 text viewer).
   ...
   This file is written based on the source ../view/ascii.c to support UTF-8 string only.
...
static inline int
domcnt_wcwidth (const WDOMContent * view, int c)
{
...
domcnt_ismark (const WDOMContent * view, int c)
{
...
static inline gboolean
domcnt_is_non_spacing_mark (const WDOMContent * view, int c)
{
    GUnicodeType type;
...
static inline gboolean
domcnt_isprint (const WDOMContent * view, int c)
{
...
static inline int
domcnt_char_display (const WDOMContent * view, int c, char *s)
{
...
static gboolean
domcnt_get_byte (WDOMContent * view, off_t byte_index, int *retval)
{
    char *p;
    ...
    if (byte_index >= 0 && byte_index < (off_t) view->text_len)
        p = view->text + byte_index;
    else
        return FALSE;
...
off_t
domcnt_bol (WDOMContent * view, off_t current, off_t limit)
{
    int c;
    off_t filesize = view->text_len;

    if (current <= 0)
        return 0;
    if (current > filesize)
        return filesize;
    if (!domcnt_get_byte (view, current, &c))
        return current;
    if (c == '\n')
    {
        if (!domcnt_get_byte (view, current - 1, &c))
            return current;
        if (c == '\r')
            current--;
    }
 ...
 * domcnt_eol of the current line == domcnt_bol next line
 */

off_t
domcnt_eol (WDOMContent * view, off_t current)
{
    int c, prev_ch = 0;
 ...
 * see ticket 3783. */
static gboolean
domcnt_get_utf (WDOMContent * view, off_t byte_index, int *ch, int *ch_len)
{
    gchar *str = NULL;
    int res;
    ...
    *ch = 0;
    ...
    if (res < 0)
    {
        ...
        {
            if (domcnt_get_byte (view, byte_index + i, &res))
                utf8buf[i] = res;
            else
            {
                utf8buf[i] = '\0';
...
/**
 * Just for convenience, a common interface in front of domcnt_get_utf and domcnt_get_byte, so that
 ...
 *
 * Just as with domcnt_get_utf(), invalid UTF-8 is reported using negative integers.
 *
 * Also, temporary hack: handle force_max here.
 * TODO: move it to lower layers (datasource.c)?
 */
static gboolean
domcnt_get_next_char (WDOMContent * view, domcnt_formatter_state_t * state, int *c)
{
    ...
    if (!domcnt_get_utf (view, state->offset, c, &char_length))
        return FALSE;
 ...
 */
static gboolean
domcnt_get_next_maybe_nroff_char (WDOMContent * view, domcnt_formatter_state_t * state, int *c, int *color)
{
    domcnt_formatter_state_t state_after_nroff;
    int c2, c3;
    ...
    if (!view->mode_flags.nroff)
        return domcnt_get_next_char (view, state, c);

    if (!domcnt_get_next_char (view, state, c))
        return FALSE;
    /* Don't allow nroff formatting around CR, LF, TAB or other special chars */
    if (!domcnt_isprint (view, *c))
        return TRUE;

    state_after_nroff = *state;

    if (!domcnt_get_next_char (view, &state_after_nroff, &c2))
        return TRUE;
    if (c2 != '\b')
        return TRUE;

    if (!domcnt_get_next_char (view, &state_after_nroff, &c3))
        return TRUE;
    if (!domcnt_isprint (view, c3))
        return TRUE;
 ...
 */
static int
domcnt_next_combining_char_sequence (WDOMContent * view, domcnt_formatter_state_t * state, int *cs,
                                     int clen, int *color)
{
    int i = 1;

    if (!domcnt_get_next_maybe_nroff_char (view, state, cs, color))
        return 0;
    ...
    if (cs[0] == '\r')
    {
        ...
        domcnt_formatter_state_t state_after_crlf = *state;
        if (domcnt_get_next_maybe_nroff_char (view, &state_after_crlf, &cnext, NULL)
            && cnext == '\n')
    ...
    if (!domcnt_isprint (view, cs[0]))
        return 1;

    if (domcnt_ismark (view, cs[0]))
    {
    ...
    for (; i < clen; i++)
    {
        domcnt_formatter_state_t state_after_combining;

        state_after_combining = *state;
        if (!domcnt_get_next_maybe_nroff_char (view, &state_after_combining, &cs[i], NULL))
            return i;
        if (!domcnt_ismark (view, cs[i]) || !domcnt_isprint (view, cs[i]))
            return i;
 ...
 * proper value. This is used by domcnt_wrap_move_down to advance in the file.
 *
 * @param view ...
 * @param state the parser-formatter state machine's state, updated
 ...
 */
static int
domcnt_display_line (WDOMContent * view, domcnt_formatter_state_t * state, int row,
                     gboolean * paragraph_ended, off_t * linewidth)
{
    const unsigned int left = view->data_area.left;
    const unsigned int top = view->data_area.top;
    const unsigned int width = view->data_area.width;
    const unsigned int height = view->data_area.height;
    ...
    if (!view->mode_flags.wrap && (row < 0 || row >= (int) height) && linewidth == NULL)
    {
        ...
        eol = domcnt_eol (view, state->offset);
        retval = (eol > state->offset) ? 1 : 0;

        domcnt_formatter_state_init (state, eol);
        return retval;
    }
    ...
    {
        int charwidth = 0;
        domcnt_formatter_state_t state_saved;
        int n;
        int color;

        state_saved = *state;
        n = domcnt_next_combining_char_sequence (view, state, cs, 1 + MAX_COMBINING_CHARS, &color);
        if (n == 0)
        {
            if (linewidth != NULL)
                *linewidth = col;
        ...
        if (cs[0] == '\n')
        {
            ...
            domcnt_formatter_state_init (state, state->offset);
            if (linewidth != NULL)
                *linewidth = col;
            return 1;
        }

        if (domcnt_is_non_spacing_mark (view, cs[0]))
        {
        ...
        if ((!domcnt_isprint (view, cs[0]) || domcnt_ismark (view, cs[0])) && cs[0] != '\t')
            cs[0] = '.';

        for (i = 0; i < n; i++)
            charwidth += domcnt_wcwidth (view, cs[i]);
        ...
        if (row >= 0 && row < (int) height)
        {
                ...
                (off_t) col + charwidth <= dpy_text_column + (off_t) width)
            {
                ...
                else
                {
                    j = 0;
                    for (i = 0; i < n; i++)
                        j += domcnt_char_display (view, cs[i], str + j);
                    str[j] = '\0';
 ...
 */
static int
domcnt_display_paragraph (WDOMContent * view, domcnt_formatter_state_t * state, int row)
{
    const unsigned int height = view->data_area.height;
    int lines = 0;
    ...
    while (TRUE)
    {
        ...
        lines += domcnt_display_line (view, state, row, &paragraph_ended, NULL);
        if (paragraph_ended)
            return lines;
 ...
 */
static void
domcnt_wrap_fixup (WDOMContent * view)
{
    ...
    domcnt_formatter_state_init (&view->dpy_state_top, view->dpy_start);

    while (lines-- != 0)
    {
        domcnt_formatter_state_t state_prev;
        gboolean paragraph_ended;

        state_prev = view->dpy_state_top;
        if (domcnt_display_line (view, &view->dpy_state_top, -1, &paragraph_ended, NULL) == 0)
            break;
        if (paragraph_ended)
        {
 ...
 */
void
domcnt_display_text (WDOMContent * view)
{
    const unsigned int left = view->data_area.left;
    const unsigned int top = view->data_area.top;
    const unsigned int height = view->data_area.height;
    int row;
    domcnt_formatter_state_t state;
    gboolean again;
    ...
    do
    {
        ...
        if (!view->mode_flags.wrap)
            domcnt_formatter_state_init (&state, view->dpy_start);
        else
        {
            domcnt_wrap_fixup (view);
            state = view->dpy_state_top;
        }

        for (row = 0; row < (int) height; row += n)
        {
            n = domcnt_display_paragraph (view, &state, row);
            if (n == 0)
            {
                ...
                {
                    domcnt_text_move_up (view, height - row);
                    again = TRUE;
                }
                break;
            }
 ...
 *
 * Note that this relies on domcnt_display_text() setting dpy_state_bottom to its correct value
 * upon rendering the screen contents. So don't call this function from other functions (e.g. at
 * the bottom of domcnt_text_move_up()) which invalidate this value.
 */
void
domcnt_text_move_down (WDOMContent * view, off_t lines)
{
    while (lines-- != 0)
    {
        gboolean paragraph_ended;
        ...
        if (domcnt_display_line (view, &view->dpy_state_bottom, -1, &paragraph_ended, NULL) == 0)
            break;
        ...
        if (!view->mode_flags.wrap)
        {
            view->dpy_start = domcnt_eol (view, view->dpy_start);
            view->dpy_paragraph_skip_lines = 0;
            view->dpy_wrap_dirty = TRUE;
        }
        else
        {
            domcnt_display_line (view, &view->dpy_state_top, -1, &paragraph_ended, NULL);
            if (!paragraph_ended)
                view->dpy_paragraph_skip_lines++;
            else
            {
 ...
 */
void
domcnt_text_move_up (WDOMContent * view, off_t lines)
{
    if (!view->mode_flags.wrap)
    {
        while (lines-- != 0)
            view->dpy_start = domcnt_bol (view, view->dpy_start - 1, 0);
        view->dpy_paragraph_skip_lines = 0;
        view->dpy_wrap_dirty = TRUE;
    }
    else
    {
        ...
        while (lines > view->dpy_paragraph_skip_lines)
        {
            ...
            view->dpy_start =
                domcnt_bol (view, view->dpy_start - 1,
            ...
            view->dpy_paragraph_skip_lines =
                domcnt_display_paragraph (view, &view->dpy_state_top, view->data_area.height);
            view->force_max = -1;
        }
        ...
        for (i = 0; i < view->dpy_paragraph_skip_lines; i++)
            domcnt_display_line (view, &view->dpy_state_top, -1, NULL, NULL);
    }
}
...
void
domcnt_text_moveto_bol (WDOMContent * view)
{
    if (!view->mode_flags.wrap)
...
void
domcnt_text_moveto_eol (WDOMContent * view)
{
    if (!view->mode_flags.wrap)
    {
        domcnt_formatter_state_t state;
        ...
        domcnt_formatter_state_init (&state, view->dpy_start);
        domcnt_display_line (view, &state, -1, NULL, &linewidth);
...
void
domcnt_formatter_state_init (domcnt_formatter_state_t * state, off_t offset)
{
