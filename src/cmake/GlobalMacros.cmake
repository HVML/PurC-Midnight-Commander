...
# PurcMC), then put it there instead.

macro(PURCMC_COMPUTE_SOURCES _framework)
...
macro(PURCMC_INCLUDE_CONFIG_FILES_IF_EXISTS)
...
macro(PURCMC_ADD_SOURCE_DEPENDENCIES _source _deps)
    set(_tmp)
...
macro(PURCMC_ADD_PRECOMPILED_HEADER _header _cpp _source)
    if (MSVC)
        ...
        foreach (_src ${_sources})
            PURCMC_ADD_SOURCE_DEPENDENCIES(${_src} ${PrecompiledBinary})
        endforeach ()
...
macro(PURCMC_WRAP_SOURCELIST)
    foreach (_file ${ARGN})
        get_filename_component(_basename ${_file} NAME_WE)
        get_filename_component(_path ${_file} PATH)

        if (NOT _file MATCHES "${DERIVED_SOURCES_PURCMC_DIR}")
            string(REGEX REPLACE "/" "\\\\\\\\" _sourcegroup "${_path}")
            source_group("${_sourcegroup}" FILES ${_file})
        endif ()
    endforeach ()

    source_group("DerivedSources" REGULAR_EXPRESSION "${DERIVED_SOURCES_PURCMC_DIR}")
endmacro()

macro(PURCMC_FRAMEWORK_DECLARE _target)
...
macro(PURCMC_EXECUTABLE_DECLARE _target)
    add_executable(${_target} "${CMAKE_BINARY_DIR}/cmakeconfig.h")
endmacro()

# Private macro for setting the properties of a target.
# Rather than just having _target like PURCMC_FRAMEWORK and PURCMC_EXECUTABLE the parameters are
# split into _target_logical_name, which is used for variable expansion, and _target_cmake_name.
# This is done to support PURCMC_WRAP_EXECUTABLE which uses the _target_logical_name variables
# but requires a different _target_cmake_name.
macro(_PURCMC_TARGET _target_logical_name _target_cmake_name)
...
macro(_PURCMC_TARGET_ANALYZE _target)
    if (ClangTidy_EXE)
...
macro(PURCMC_FRAMEWORK _target)
    _PURCMC_TARGET(${_target} ${_target})
    _PURCMC_TARGET_ANALYZE(${_target})
...
# FIXME Move into PURCMC_FRAMEWORK after all libraries are using this macro
macro(PURCMC_FRAMEWORK_TARGET _target)
    ...
    add_library(PurcMC::${_target} ALIAS ${_target}_PostBuild)
endmacro()

macro(PURCMC_EXECUTABLE _target)
    _PURCMC_TARGET(${_target} ${_target})
    _PURCMC_TARGET_ANALYZE(${_target})
...
macro(PURCMC_WRAP_EXECUTABLE _target)
    ...
    _PURCMC_TARGET(${_target} ${_wrapped_target_name})
    _PURCMC_TARGET_ANALYZE(${_wrapped_target_name})
...
macro(PURCMC_CREATE_FORWARDING_HEADER _target_directory _file)
    ...
    # PurcMC2 generate-forwarding-headers script.
...
macro(PURCMC_CREATE_FORWARDING_HEADERS _framework)
    ...
    if (NOT WIN32)
        ...
        foreach (_currentArg ${ARGN})
            ...
            elseif (_processing_directories)
                ...
                foreach (_file ${_files})
                    PURCMC_CREATE_FORWARDING_HEADER(${_target_directory} ${_file})
                endforeach ()
            elseif (_processing_files)
                PURCMC_CREATE_FORWARDING_HEADER(${_target_directory} ${_currentArg})
            endif ()
        endforeach ()
    endif ()
endmacro()

function(PURCMC_MAKE_FORWARDING_HEADERS framework)
...
function(PURCMC_COPY_FILES target_name)
    set(options FLATTENED)
...
macro(PURCMC_DEBUG_DUMP_COMMANDS)
    set(CMAKE_VERBOSE_MAKEFILE ON)
endmacro()

macro(PURCMC_DEBUG_DUMP_VARIABLES)
...
macro(PURCMC_ADD_TARGET_PROPERTIES _target _property _flags)
...
macro(PURCMC_POPULATE_LIBRARY_VERSION library_name)
...
macro(PURCMC_CREATE_SYMLINK target src dest)
